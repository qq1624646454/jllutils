...
wget http://192.168.1.56/userRpm/StatusRpm.html --user=jielong_lin --password=jll111111KBD
if [ -e "$(pwd)/StatusRpm.html" ]; then
    MyDynamicIP="$(sed -n '46p' $(pwd)/StatusRpm.html | sed 's/,//')"
    rm -rvf $(pwd)/StatusRpm.html
    echo
    echo
    echo "${MyDynamicIP}"
    echo
