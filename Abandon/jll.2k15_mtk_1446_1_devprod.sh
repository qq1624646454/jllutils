...
function Fn_PhilipsTV_Compilation()
{
    ...
    # Checking if the project is valid
    #
    LvpcVariable="${GvPrjRootPath}/device/tpvision"
    LvpcExecShell="${LvpcVariable}/common/sde/upg/build_philipstv.sh"
    ...
    while [ 1 -eq 1 ]; do
        ...
        LvpcOptionProduct=$(echo "${LvpcOptionProduct}" | grep -v "THIS_BUILD=${LvpcResult}")
    ...
    if [ x"${LvpcChoice}" = x"y" ]; then
        cd ${LvpcCmdPath}
        echo "./${LvpcCmdName} -p ${LvpcOptionProduct} -b ${LvpcOptionBuild} -n 8 "
        echo
