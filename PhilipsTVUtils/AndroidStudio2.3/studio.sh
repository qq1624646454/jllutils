...
export DISPLAY=xmnb4003161.tpvaoc.com:0.0

# JLL.S20170718: Customize to start AVD before AS
#  ls ~/.android/avd | awk -F'.ini' '{print $1}' | awk -F'.avd' '{print $1}' | uniq
#  read -p "Please Type above AVD_name=" _as_AVD_name
#  [ x"${_as_AVD_name}" != x ] && ~/dl.google.com/android/repository/emulator/emulator \
#  -avd "${_as_AVD_name}" -gpu swiftshader &
#  __AVD_PID=$!
# JLL.E20170718: Customize to start AVD before AS
...
# JLL.S20170718: Customize to start AVD before AS
#if [ x"${__AVD_PID}" != x ]; then
#    __is_Alive=$(ps -e | awk -F' ' '{print $1}' | grep "${__AVD_PID}" 2>/dev/null)
#    if [ x"${__is_Alive}" != x ]; then
#        echo "JLL: AVD should be killed. AVD_PID=${__AVD_PID}"
#        kill -9 ${__AVD_PID}
#    fi
#fi
# JLL.E20170718: Customize to start AVD before AS
