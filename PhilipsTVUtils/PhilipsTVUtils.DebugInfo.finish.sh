...
if [ -e "${GvUDisk}/TPVisionDebug.cookie" ]; then
    if [ -e "${GvUDisk}/TPVisionDebug.cookie.jll" ]; then
        rm -rf ${GvUDisk}/TPVisionDebug.cookie
    else
        mv -vf ${GvUDisk}/TPVisionDebug.cookie ${GvUDisk}/TPVisionDebug.cookie.jll
    fi
fi


### If the number of TPVisionDebug.XXX folders more than 5, First will be removed.
GvList=$(ls ${GvUDisk} | grep -i "TPVisionDebug")
GvIdx=0
for GvL in ${GvList}; do
    ((GvIdx+=1))
done
if [ ${GvIdx} -gt 5 ]; then
    for GvL in ${GvList}; do
