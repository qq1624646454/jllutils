...
#   ModifiedTime: 2017-06-06 09:58:34
JLLPATH="$(which $0)"
                ...
                for((iSR=0;iSR<__iSegment;iSR+=2)) {
                    ...
                    for((iKW=0;iKW<__iRanges;iKW+=3)) {
                        if [ ${__lstRanges[iKW+1]} -ge ${__iRSP} \
                            ...
                            [ ${__iKEP} -le ${__iKSP} ] &&  __iKEP=$((iKW+1))
                        fi


                        if [ ${__iKSP} -lt 1 ]; then
                            if [ ${__lstRanges[iKW+1]} -ge ${__iRSP} ]; then
                                __iKSP=$((iKW+1))
                                __iKEP=${__iKSP}
                                continue
                            fi
                        else 
                            if [ ${__iKEP} -eq ${__iKSP} ]; then
                                if [ ${__lstRanges[iKW+1]} -lt ${__iRSP} ]; then
                                    __iKEP=$((iKW+1))
                                    continue
                                fi
                            fi
